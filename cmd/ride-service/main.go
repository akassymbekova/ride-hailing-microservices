...
func Run() {
	...
	repo := ride.NewPostgresRepository(pool)
	events := ride.NewEventPublisher(publisher)
	hub := ws.NewHub(log, nil)
	service := ride.NewService(repo, events, hub, log)
	consumer := ride.NewConsumer(service, log)
	consumer.Start(ctx, rabbitConn)

	wsHandler := ride.NewWebSocketHandler(hub, log)
	handler := ride.NewHandler(service, wsHandler)

	mux := http.NewServeMux()
	handler.RegisterRoutes(mux)

	serverPort := getEnv("RIDE_SERVICE_PORT", "3000")
	srv := &http.Server{
		Addr:         ":" + serverPort,
		Handler:      ride.RequestIDMiddleware(mux),
		ReadTimeout:  10 * time.Second,
		WriteTimeout: 10 * time.Second,
	}

	go func() {
		log.Info(ctx, "server_started", "ride-service listening", "port", serverPort)
	...
	log.Info(context.Background(), "shutdown_complete", "ride-service stopped gracefully")
}
